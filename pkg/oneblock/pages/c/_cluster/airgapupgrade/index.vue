...
export default {
  ...
  async fetch() {
    await this.$store.dispatch('oneblock/findAll', { type: OB.IMAGE });

    const value = await this.$store.dispatch('oneblock/create', {
      type:     OB.UPGRADE,
      metadata: {
    ...
    const imageValue = await this.$store.dispatch('oneblock/create', {
      type:     OB.IMAGE,
      metadata: {
        name:         '',
        namespace:    'harvester-system',
  ...
  computed: {
    ...
    osImageOptions() {
      return this.$store.getters['oneblock/all'](OB.IMAGE)
  ...
  methods: {
      ...
      this.$router.push({
        name:   this.doneRoute,
        params: { resource: OB.SETTING, product: 'harvester' }
      });
    },
    ...
    async save(buttonCb) {
      ...
      try {
        if (this.imageSource === IMAGE_METHOD.NEW) {
          this.imageValue.metadata.annotations[OB_ANNOTATIONS.OS_UPGRADE_IMAGE] = 'True';
          ...
          if (this.sourceType === UPLOAD) {
            ...
            this.imageValue.metadata.annotations[OB_ANNOTATIONS.IMAGE_NAME] = file.name;
