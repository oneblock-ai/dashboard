...
import ResourceSummary, { resourceCounts } from '@pkg/oneblock/components/ResourceSummary';
...
const SUM_RESOURCES = [{
  ...
  type:    'ray.io.raycluster',
  spoofed: {
    location: {
      name:   `${ PRODUCT }-c-cluster-resource`,
      params: { resource: 'ray.io.raycluster' }
    },
    name: 'ray.io.raycluster',
  },
  resource: 'ray.io.raycluster'
}, {
  type:    'ml.oneblock.ai.notebook',
  spoofed: {
    location: {
      name:   `${ PRODUCT }-c-cluster-resource`,
      params: { resource: 'ml.oneblock.ai.notebook' }
    },
    name: 'ml.oneblock.ai.notebook',
  },
  resource: 'ml.oneblock.ai.notebook',
}];
...
export default {
  ...
  computed: {
    ...
    productVersion() {
      const settings = this.$store.getters['oneblock/all']('management.oneblock.ai.setting') || [];
      const version = settings.find((setting) => {
        return setting.id === 'server-version';
      }).default;
...
<template>
  <section class="dashboard">
    <header>
      <div class="title">
        <h1>
          {{ currentCluster.spec.displayName || currentCluster.metadata.name }}
        </h1>
        <div>
    ...
    <div class="resource-gauges">
      <ResourceSummary
        v-for="resource in SUM_RESOURCES"
        :key="resource.resource"
        :resource="resource.resource"
        :spoofed-location="resource.spoofed.location"
      />
    </div>
