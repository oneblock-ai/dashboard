...
{
  type:    OB.BLOCK_DEVICE,
  spoofed: {
    location: {
      name:   `${ HARVESTER_PRODUCT }-c-cluster-resource`,
      params: { resource: OB.HOST }
    },
    name: OB.BLOCK_DEVICE,
  },
}];
...
export default {
  ...
  async fetch() {
    ...
    const hash = {
      vms:          this.fetchClusterResources(OB.VM),
      ...
      settings:     this.fetchClusterResources(OB.SETTING),
      ...
      _pods:        this.$store.dispatch('oneblock/findAll', { type: POD }),
    };
    ...
    if (this.$store.getters[`${ inStore }/schemaFor`](OB.ADD_ONS)) {
      hash.addons = this.$store.dispatch(`${ inStore }/findAll`, { type: OB.ADD_ONS });
    }
  ...
  computed: {
      ...
      this.accessibleResources.forEach((resource) => {
        ...
        if (resource.spoofed) {
          if (resource.spoofed?.filterNamespace && Array.isArray(resource.spoofed.filterNamespace)) {
            const clusterCounts = this.$store.getters['oneblock/all'](COUNT)[0].counts;
        ...
        if (resource.type === OB.BLOCK_DEVICE) {
          let total = 0;
          let errorCount = 0;
    ...
    currentVersion() {
      const inStore = this.$store.getters['currentProduct'].inStore;
      const settings = this.$store.getters[`${ inStore }/all`](OB.SETTING);
    ...
    toEnableMonitoringAddon() {
      return `${ OB.ADD_ONS }/cattle-monitoring-system/rancher-monitoring?mode=edit#alertmanager`;
    },

    canEnableMonitoringAddon() {
      const inStore = this.$store.getters['currentProduct'].inStore;
      const hasSchema = this.$store.getters[`${ inStore }/schemaFor`](OB.ADD_ONS);
