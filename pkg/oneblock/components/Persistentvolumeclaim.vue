...
export default {
  name:       'OneBlockPersistentVolumeClaim',
  components: {
  ...
  async fetch() {
    const hash = await allHash({
      storageClasses:    this.$store.dispatch('oneblock/findAll', { type: STORAGE_CLASS }),
      persistentVolumes: this.$store.dispatch('oneblock/findAll', { type: PV }),
    });
