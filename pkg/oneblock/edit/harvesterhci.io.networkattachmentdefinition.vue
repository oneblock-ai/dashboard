...
export default {
  ...
  data() {
    ...
    const layer3Network = JSON.parse(annotations[OB_LABELS_ANNOTATIONS.NETWORK_ROUTE] || '{}');
  ...
  async fetch() {
    ...
    await allHash({ clusterNetworks: this.$store.dispatch(`${ inStore }/findAll`, { type: OB.CLUSTER_NETWORK }) });
  },
  ...
  methods: {
    async saveNetwork(buttonCb) {
      ...
      this.value.setAnnotation(OB_LABELS_ANNOTATIONS.NETWORK_ROUTE, JSON.stringify(this.layer3Network));
