...
import { OB_ALLOWED_SETTINGS, OB_SINGLE_CLUSTER_ALLOWED_SETTING, OB_SETTING } from '../config/settings';
...
export default {
  ...
  data() {
    ...
    const setting =
      OB_ALLOWED_SETTINGS[this.value.id] ||
      OB_SINGLE_CLUSTER_ALLOWED_SETTING[this.value.id];
  ...
  created() {
    ...
    this.registerAfterHook(() => {
      if (this.value.id === OB_SETTING.RANCHER_MANAGER_SUPPORT) {
  ...
  methods: {
    ...
    async saveSettings(done) {
      ...
      if (this.value.metadata.name === OB_SETTING.CLUSTER_REGISTRATION_URL && this.oldValue && this.value.value !== this.oldValue) {
    ...
    clusterRegistrationUrlTip() {
      return new Promise((resolve) => {
        this.$store.dispatch('oneblock/promptModal', {
    ...
    useDefault(ev) {
      ...
      if (this.value.id === OB_SETTING.VLAN) {
