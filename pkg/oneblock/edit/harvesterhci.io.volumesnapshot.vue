...
import { OB as OB_ANNOTATIONS } from '@shell/config/labels-annotations';
...
export default {
  ...
  computed: {
    ...
    isImageVolume() {
      return !!get(this.value, `metadata.annotations."${ OB_ANNOTATIONS.IMAGE_ID }"`);
    },

    imageName() {
      const image = this.$store.getters['oneblock/all'](OB.IMAGE).find((I) => {
        return I.id === get(this.value, `metadata.annotations."${ OB_ANNOTATIONS.IMAGE_ID }"`);
      });
