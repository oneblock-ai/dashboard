...
export default class HciSetting extends HarvesterResource {
  get _availableActions() {
    ...
    const settingMetadata = OB_ALLOWED_SETTINGS[this.id];
    ...
    const schema = this.$getters['schemaFor'](OB.UPGRADE);
    const hasUpgradeAccess = !!schema?.collectionMethods.find((x) => ['post'].includes(x.toLowerCase()));

    if (this.id === OB_SETTING.SERVER_VERSION && hasUpgradeAccess) {
      const latestUpgrade = this.$getters['all'](OB.UPGRADE).find((upgrade) => upgrade.isLatestUpgrade);
  ...
  get customValue() {
    if (this.metadata.name === OB_SETTING.STORAGE_NETWORK) {
      try {
