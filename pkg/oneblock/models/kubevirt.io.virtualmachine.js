...
export default class VirtVm extends HarvesterResource {
  ...
  applyDefaults(resources = this, realMode) {
    ...
    if (realMode !== _CLONE) {
      Vue.set(this.metadata, 'annotations', { [OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]: '[]' });
  ...
  cleanForNew() {
    ...
    try {
      volumeClaimTemplate = JSON.parse(this.metadata.annotations[OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]);
    } catch (e) {}
    ...
    this.metadata.annotations[OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE] = JSON.stringify(deleteDataSource);
  }
  ...
  get realAttachNodeName() {
    const vmi = this.$getters['byId'](OB.VMI, this.id);
  ...
  get podResource() {
    ...
    const vmiResource = this.$rootGetters[`${ inStore }/byId`](OB.VMI, this.id);
  ...
  get vmi() {
    ...
    const vmis = this.$rootGetters[`${ inStore }/all`](OB.VMI);
  ...
  get restoreResource() {
    const id = `${ this.metadata.namespace }/${ get(
      this,
      `metadata.annotations."${ OB_ANNOTATIONS.RESTORE_NAME }"`
    ) }`;

    const inStore = this.productInStore;

    const allRestore = this.$rootGetters[`${ inStore }/all`](OB.RESTORE);
  ...
  get warningMessage() {
    if (this.metadata?.annotations[OB_ANNOTATIONS.VM_INSUFFICIENT]) {
      return {
        message:    this.metadata?.annotations[OB_ANNOTATIONS.VM_INSUFFICIENT],
        canDismiss: true,
      };
    }
  ...
  get resourcesStatus() {
    const inStore = this.productInStore;
    const vmList = this.$rootGetters[`${ inStore }/all`](OB.VM);
    let warningCount = 0;
    let errorCount = 0;
  ...
  get volumeClaimTemplates() {
    ...
    try {
      out = JSON.parse(
        this.metadata?.annotations?.[OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]
      );
    } catch (e) {}
  ...
  get restoreName() {
    return (
      get(this, `metadata.annotations."${ OB_ANNOTATIONS.RESTORE_NAME }"`) || ''
    );
  }
