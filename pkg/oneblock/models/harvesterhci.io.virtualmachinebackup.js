...
export default class HciVmBackup extends HarvesterResource {
  detailPageHeaderActionOverride(realMode) {
    ...
    if (realMode === _CREATE) {
      return route.params.resource === OB.BACKUP ? this.t('harvester.backup.title') : this.t('harvester.vmSnapshot.title');
    }
  }
  ...
  get attachVmExisting() {
    const vmList = this.$rootGetters['oneblock/all'](OB.VM);
