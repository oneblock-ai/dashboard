...
export default class HciVmImage extends HarvesterResource {
  ...
  applyDefaults(resources = this, realMode) {
    if (realMode !== _CLONE) {
      Vue.set(this.metadata, 'labels', { [OB_ANNOTATIONS.OS_TYPE]: '', [OB_ANNOTATIONS.IMAGE_SUFFIX]: '' });
      Vue.set(this.metadata, 'annotations', { [OB_ANNOTATIONS.STORAGE_CLASS]: '' });
    }
  }
  ...
  get isOSImage() {
    return this?.metadata?.annotations?.[OB_ANNOTATIONS.OS_UPGRADE_IMAGE] === 'True';
  }
  ...
  get imageSuffix() {
    return this.metadata?.labels?.[OB_ANNOTATIONS.IMAGE_SUFFIX];
  }

  get imageOSType() {
    return this.metadata?.labels?.[OB_ANNOTATIONS.OS_TYPE];
  }
