...
import { OB } from '@pkg/oneblock/config/labels-annotations';
import HarvesterResource from './harvester';

export default class HciUpgrade extends HarvesterResource {
  get isLatestUpgrade() {
    return this?.metadata?.labels?.[OB.LATEST_UPGRADE] === 'true';
  }

  get isUpgradeSucceeded() {
    return this?.metadata?.labels?.[OB.UPGRADE_STATE] === 'Succeeded';
  }

  get hasReadMessage() {
    return this?.metadata?.labels?.[OB.REAY_MESSAGE] === 'true';
  }
  ...
  get nodes() {
    return this.$rootGetters['oneblock/all'](NODE);
  }
