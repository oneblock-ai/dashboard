...
export default {
  methods: {
    ...
    getSSHValue(id) {
      ...
      const sshs = this.$store.getters[`${ inStore }/all`](OB.SSH) || [];

      return sshs.find( (O) => O.id === id)?.spec?.publicKey || undefined;
    },

    getOsType(vm) {
      return vm.metadata?.labels?.[OB_ANNOTATIONS.OS];
    },
    ...
    getAccessCredentials(spec) {
      ...
      const annotations = JSON.parse(spec.template.metadata?.annotations?.[OB_ANNOTATIONS.DYNAMIC_SSHKEYS_NAMES] || '[]');
    ...
    getVolumeClaimTemplates(vm) {
      ...
      try {
        out = JSON.parse(vm.metadata.annotations[OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]);
      } catch (e) {
    ...
    getRootImageId(vm) {
      ...
      return volume[0]?.metadata?.annotations?.[OB_ANNOTATIONS.IMAGE_ID] || '';
    },

    getSSHFromAnnotation(spec) {
      const ids = spec?.template?.metadata?.annotations?.[OB_ANNOTATIONS.SSH_NAMES] || '[]';
    ...
    mergeAllSSHs(spec) {
      ...
      const inStore = this.$store.getters['currentProduct'].inStore;
      const allSSHs = this.$store.getters[`${ inStore }/all`](OB.SSH) || [];
