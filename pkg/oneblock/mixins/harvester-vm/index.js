...
import { OB_SETTING } from '../../config/settings';
...
export default {
  ...
  async fetch() {
    ...
    const hash = {
      ...
      sshs:              this.$store.dispatch(`${ inStore }/findAll`, { type: OB.SSH }),
      settings:          this.$store.dispatch(`${ inStore }/findAll`, { type: OB.SETTING }),
      images:            this.$store.dispatch(`${ inStore }/findAll`, { type: OB.IMAGE }),
      versions:          this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VM_VERSION }),
      templates:         this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VM_TEMPLATE }),
      networkAttachment: this.$store.dispatch(`${ inStore }/findAll`, { type: NETWORK_ATTACHMENT }),
      vmis:              this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VMI }),
      vmims:             this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VMIM }),
      vms:               this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VM }),
    ...
    if (this.$store.getters[`${ inStore }/schemaFor`](OB.CLUSTER_NETWORK)) {
      hash.clusterNetworks = this.$store.dispatch(`${ inStore }/findAll`, { type: OB.CLUSTER_NETWORK });
    }

    if (this.$store.getters[`${ inStore }/schemaFor`](OB.VLAN_CONFIG)) {
      hash.clusterNetworks = this.$store.dispatch(`${ inStore }/findAll`, { type: OB.VLAN_CONFIG });
    }
    ...
    const hasPCISchema = !!this.$store.getters[`${ inStore }/schemaFor`](OB.PCI_DEVICE);
    const hasSRIOVGPUSchema = !!this.$store.getters[`${ inStore }/schemaFor`](OB.SR_IOVGPU_DEVICE);
  ...
  computed: {
    ...
    images() {
      return this.$store.getters[`${ this.inStore }/all`](OB.IMAGE);
    },

    versions() {
      return this.$store.getters[`${ this.inStore }/all`](OB.VM_VERSION);
    },

    templates() {
      return this.$store.getters[`${ this.inStore }/all`](OB.VM_TEMPLATE);
    },
    ...
    filteredNamespaces() {
      return this.$store.getters['oneblock/all'](NAMESPACE).filter((namespace) => !namespace.isSystem);
    },

    nodes() {
      return this.$store.getters['oneblock/all'](NODE);
    },
    ...
    storageClassSetting() {
      try {
        const storageClassValue = this.$store.getters[`${ this.inStore }/all`](OB.SETTING).find( (O) => O.id === OB_SETTING.DEFAULT_STORAGE_CLASS)?.value;
    ...
    needNewSecret() {
      ...
      return this.resource === OB.VM_VERSION || this.isCreate;
    },

    defaultTerminationSetting() {
      const setting = this.$store.getters[`${ this.inStore }/all`](OB.SETTING).find( (O) => O.id === OB_SETTING.VM_TERMINATION_PERIOD) || {};
  ...
  methods: {
    getInitConfig(config) {
      ...
      const vm = this.resource === OB.VM ? value : this.resource === OB.BACKUP ? this.value.status?.source : value.spec.vm;
      ...
      const reservedMemory = vm.metadata?.annotations?.[OB_ANNOTATIONS.VM_RESERVED_MEMORY];
      ...
      if (_disks.length === 0) {
        ...
        out = _disks.map( (DISK, index) => {
          ...
          if (volume.persistentVolumeClaim && volume.persistentVolumeClaim?.claimName) {
            ...
            if (DVT) {
              // has annotation (OB_ANNOTATIONS.IMAGE_ID) => SOURCE_TYPE.IMAGE
              if (DVT.metadata?.annotations?.[OB_ANNOTATIONS.IMAGE_ID] !== undefined) {
                image = DVT.metadata?.annotations?.[OB_ANNOTATIONS.IMAGE_ID];
                source = SOURCE_TYPE.IMAGE;
              } else {
              ...
              const allPVCs = this.$store.getters['oneblock/all'](PVC);
          ...
          const volumeStatus = this.pvcs.find((P) => P.id === `${ this.value.metadata.namespace }/${ volumeName }`)?.relatedPV?.metadata?.annotations?.[OB_ANNOTATIONS.VOLUME_ERROR];
    ...
    parseOther() {
      ...
      const vm = this.resource === OB.VM ? this.value : this.value.spec.vm;

      if (!this.reservedMemory) {
        delete vm.metadata.annotations[OB_ANNOTATIONS.VM_RESERVED_MEMORY];
      } else {
        vm.metadata.annotations[OB_ANNOTATIONS.VM_RESERVED_MEMORY] = this.reservedMemory;
      }
    },
    ...
    parseDiskRows(disk) {
        ...
        template:    {
          ...
          metadata: {
            ...
            annotations: {
              ...
              [OB_ANNOTATIONS.SSH_NAMES]: JSON.stringify(this.sshKey)
            },
            labels: {
              ...this.spec?.template?.metadata?.labels,
              [OB_ANNOTATIONS.VM_NAME]: this.value?.metadata?.name,
            }
          },
          spec: {
      ...
      if (this.resource === OB.VM) {
        ...
        this.$set(this.value.metadata, 'annotations', {
          ...
          [OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]: JSON.stringify(volumeClaimTemplates),
          [OB_ANNOTATIONS.NETWORK_IPS]:           JSON.stringify(this.value.networkIps)
        });

        this.$set(this.value.metadata, 'labels', {
          ...this.value.metadata.labels,
          [OB_ANNOTATIONS.CREATOR]: 'harvester',
          [OB_ANNOTATIONS.OS]:      this.osType
        });

        this.$set(this.value, 'spec', spec);
        this.$set(this, 'spec', spec);
      } else if (this.resource === OB.VM_VERSION) {
        this.$set(this.value.spec.vm, 'spec', spec);
        this.$set(this.value.spec.vm.metadata, 'annotations', { ...this.value.spec.vm.metadata.annotations, [OB_ANNOTATIONS.VOLUME_CLAIM_TEMPLATE]: JSON.stringify(volumeClaimTemplates) });
    ...
    multiVMScheduler(spec) {
      ...
      spec.template.metadata.labels[OB_ANNOTATIONS.VM_NAME_PREFIX] = namePrefix;

      const rule = {
        weight:          1,
        podAffinityTerm: {
          topologyKey:   HOSTNAME,
          labelSelector: { matchLabels: { [OB_ANNOTATIONS.VM_NAME_PREFIX]: namePrefix } }
        }
      };
    ...
    parseAccessCredentials() {
      const out = [];
      const annotations = {};
      const users = JSON.parse(this.spec?.template?.metadata?.annotations?.[OB_ANNOTATIONS.DYNAMIC_SSHKEYS_USERS] || '[]');
      ...
      if (users.length !== 0) {
        this.spec.template.metadata.annotations[OB_ANNOTATIONS.DYNAMIC_SSHKEYS_USERS] = JSON.stringify(Array.from(new Set(users)));
        this.spec.template.metadata.annotations[OB_ANNOTATIONS.DYNAMIC_SSHKEYS_NAMES] = JSON.stringify(annotations);
      }
    },
    ...
    parseVolumeClaimTemplate(R, dataVolumeName) {
      ...
      case SOURCE_TYPE.IMAGE: {
        ...
        if (image) {
          ...
          out.metadata.annotations = { [OB_ANNOTATIONS.IMAGE_ID]: image.id };
        } else {
          out.metadata.annotations = { [OB_ANNOTATIONS.IMAGE_ID]: '' };
        }
    ...
    deleteQGA(config) {
      ...
      const userDataTemplateValue = this.$store.getters['oneblock/byId'](CONFIG_MAP, this.userDataTemplateId)?.data?.cloudInit || '';
    ...
    getOwnerReferencesFromVM(resource) {
      ...
      const apiVersion = this.resource === OB.VM ? 'kubevirt.io/v1' : 'harvesterhci.io/v1beta1';
    ...
    async saveSecret(vm) {
      ...
      if (!secret || this.needNewSecret) {
        secret = await this.$store.dispatch('oneblock/create', {
          metadata: {
            name:            this.secretName,
            namespace:       this.value.metadata.namespace,
            labels:          { [OB_ANNOTATIONS.CLOUD_INIT]: 'harvester' },
    ...
    async saveAccessCredentials(vm) {
      ...
      for (const row of this.accessCredentials) {
        ...
        if (!secretRef || this.needNewSecret) {
          secretRef = await this.$store.dispatch('oneblock/create', {
            metadata: {
              name:            row.secretName,
        ...
        if (row.source === ACCESS_CREDENTIALS.INJECT_SSH) {
          for (const secretId of row.sshkeys) {
            const keypair = (this.$store.getters['oneblock/all'](OB.SSH) || []).find((s) => s.id === secretId);
  ...
  watch: {
    ...
    secretRef: {
      handler(secret) {
        if (secret && this.resource !== OB.BACKUP) {
