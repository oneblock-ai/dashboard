...
export default {
  async loadCluster(
    ...
    if (getters['schemaFor'](OB.UPGRADE)) {
      hash.upgrades = dispatch('findAll', { type: OB.UPGRADE });
    }
    ...
    const isMultiCluster = !(
      res.clusters.length === 1 && res.clusters[0].metadata?.name === 'local'
    );
